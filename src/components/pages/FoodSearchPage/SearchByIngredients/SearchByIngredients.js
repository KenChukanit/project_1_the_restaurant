...
function SearchByIngredients(props) {
    ...
    onInputValueChange: ({ inputValue }) => {
      setInputItems(
        foods.filter((item) =>
          item.ingredients.includes(inputValue)
        )
      )
    },
  })
  ...
  return (
    <main >
      <h2>Please search by Ingredients: {singleFood}</h2>
        ...
        {isOpen &&
          inputItems.map((item, index) => (
            <span
              ...
              onClick={() => setSingleFood(item.ingredients)}
            >
